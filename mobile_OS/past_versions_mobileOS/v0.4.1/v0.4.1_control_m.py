...
class UmaGame:
    ...
    def _roll_prize_derby(self, times: int = 100):
        for i in range(times):
            self.click(1550, 360, 0.1)
            self.click(1550, 200, 0.1)

    def _start_game(self, character: HorseGirl, parent: tuple, mode: bool = 0):
        """Starting game from home screen."""
        self.nclick(1500, 400, 2, 1.5)
        if mode:  # To continue a game.
            ...
            self.click(1450, 450) 
            self.wait_for(f"parent/{parent[0]}", click=1)  # Select first parent
            self.click(1550, 610)  # Confirm
            self.click(1600, 450)
            self.wait_for(f"parent/{parent[1]}", click=1)  # Select second parent
    ...
    def _trouble_shoot(self, racemode=0):
        ...
        if self.trouble_count >= 9:
            exit("Cannot resolve trouble, ending autoplay.")
        elif self.trouble_count <= 1 and racemode == 0:
            return 0
        try: 
        ...
        if racemode:
            time.sleep(0.5)
            if self.wait_for("generaltraining/OK", rg=(1590, 470, 80, 80), click=1, timeout=0.5, confi=0.6):
                return 0
        try:
    ...
    def _check_mainrace(self):
        ...
        self.click(1620, 520)
        self.__change_running_style__(self._style_at_turn())
        if self.wait_for("generaltraining/Result", rg=(1456, 640, 100, 40), click=1) is None:
            raise NotImplementedError
        self.clicks_until((1565, 660), "generaltraining/Option", rg=option_bar, timeout=3)
        self._try_again()
        self.clicks_until((1565, 660), "generaltraining/Option", rg=option_bar, timeout=12)
        time.sleep(1.5)
        if self.turn >= 76:
    ...
    def _try_again(self):
        if self.wait_for("generaltraining/try_again", rg=(1570, 505, 100, 40), click=1, timeout=0.5) is None:
            return 0
        if self.wait_for("generaltraining/Result", rg=(1456, 640, 100, 40), click=1) is None:
            raise NotImplementedError


    def _style_at_turn(self):
        if self.turn in self.c.strategy.keys():
            return self.c.strategy[self.turn]
        else:
            return self.c.strategy[1]

    def __change_running_style__(self, style: str = "Front"):  # Need improvements
        if self.style != style:
            time.sleep(7)
            self.click(1635, 455, 1)  # change to style. 
            t = {"Front": 1635, "Pace": 1580, "Late": 1515, "End": 1455}
            self.click(t[style], 455, 1)
            self.click(1620, 520)
            self.style = style
    ...
    def _infirmary(self):
        if self.test_image("generaltraining/Infirmary", confi=0.9998, rg=control_panel):  # Go to the infirmary to treat
    ...
    def _check_mood(self):
        ...
        if not (self.test_image("generaltraining/Training", confi=0.99, rg=control_panel) or self.mood_checked):
            self._check_multiq()
            self._trouble_shoot()
            self.mood_checked = 1
            self._check_mood()
        ...
        for i in bad_mood:
            if self.test_image(f"generaltraining/{i}", rg=mood_bar):
                self.__raise_mood__()
                raise ContinueException
            else:
                pass
        if self.test_image(f"generaltraining/Good", rg=mood_bar):
            return 1
        else:
            return 0
    ...
    def _check_race(self, rl: dict = None):
        ...
        if rl is None:
            ...
            rb1 = race_barS if self.turn in summer_training else race_bar  # Races label is in different position during S training.
        if self.turn in rl.keys():
            if not (self.test_image(f"generaltraining/Races", rg=rb1) or self.test_image(f"URA/races/{rl[self.turn]}", rg=race_selection_bar)):
                print("Trouble shooting for race preparation...")
                self._check_multiq()
                self._trouble_shoot(1)
                self._check_race()
            self.wait_for(f"generaltraining/Races", rg=rb1, click=1)
            self._trouble_shoot(1)
            if self.wait_for(f"URA/races/{rl[self.turn]}", click=1, rg=race_selection_bar) is None:
                self.click(1560, 445)
            ...
            self.wait_for(f"generaltraining/Race", rg=(1580, 610, 100, 30), click=1)
            self.click(1620, 520)
    ...
    def _check_energy(self):
        if self.wait_for(f"generaltraining/Training", confi=0.99, timeout=3, rg=control_panel) is None:
            return 0
    ...
    def _check_training(self, mood_score: float):
        ...
        if self.wait_for("generaltraining/Training", timeout=7, click=1, rg=control_panel) is None:
            return None

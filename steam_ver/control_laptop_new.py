...
from core import (
    GameWindow, GameTurn, WaitTime, ImagePath, MoodScore,
    Coordinate, ContinueException, WindowBounds
)
...
from events import (
    EventHandlers, MoodManager, HealthManager,
    RaceManager, TrainingManager
)
...
class UmaGame:
    ...
    def __init__(
        ...
        self.character.pre_trainoption = 0
    ...
    def execute_ura_training(self) -> None:
        ...
        HealthManager.check_for_infirmary(
            self.click, self.cfg, self.turn
        )
        HealthManager.check_energy_level(
            self.click, self.cfg, self.turn
        )
        
        mood_score = MoodScore.GOOD if MoodManager.check_mood(
            self.turn,
        ...
        TrainingManager.select_best_training(
            self.turn, mood_score, self.character,
            self.click, self.click_multiple,
            lambda: MoodManager.raise_mood(self.click, self.cfg, self.turn),
            lambda: EventHandlers.check_for_date_event(
                EventHandlers.detect_event_type,
                lambda c: EventHandlers.handle_special_event(
        ...
        self.click_multiple(
            self.cfg["lobby_ui"]["view_result_button"], 3, result_wait
        )
    ...
    def execute_daily_training(self) -> None:
        ...
        RaceManager.handle_scheduled_race(
            ...
            lambda: RaceManager.upgrade_skills(
                self.window_bounds, self.wait_for_image,
                self.click, self.screen_reader, self.cfg
            ),
            self.wait_for_image,
            self.click,
            self.click_multiple,
    ...
    def handle_manual_race(self) -> None:
        ...
        RaceManager.change_race_strategy(
            self.turn, self.character, self.wait_for_image
        )
                ...
                if self.turn >= GameTurn.URA_START_TURN:
                    ...
                    if coord:
                        EventHandlers.handle_special_event(
                            coord, self.list_event, self.cfg,
                            self.screen_reader, self.turn
                        )
                    self.execute_daily_training()
